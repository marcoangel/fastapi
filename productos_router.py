...
from seguridad import obternerUsuarioActual
...
def crear_producto(producto: ProductoBase, db: Session = Depends(get_db), usuario_actual: str = Depends(obternerUsuarioActual)):
...
def delete_producto(producto_id: int, db: Session = Depends(get_db), usuario_actual: str = Depends(obternerUsuarioActual)):
    producto = db.query(ProductoDB).filter(ProductoDB.id == producto_id).first()
    if not producto:
        raise HTTPException(status_code=404, detail="Producto no encontrado")
    db.delete(producto)
